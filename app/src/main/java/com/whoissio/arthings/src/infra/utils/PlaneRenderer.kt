...
import com.whoissio.arthings.src.infra.utils.ShaderUtil.checkGLError
import com.whoissio.arthings.src.infra.utils.ShaderUtil.loadGLShader
...
class PlaneRenderer {
  ...
  fun createOnGlThread(context: Context, gridDistanceTextureName: String) {
    val vertexShader = loadGLShader(context, GLES20.GL_VERTEX_SHADER, VERTEX_SHADER_NAME)
    val passthroughShader = loadGLShader(context, GLES20.GL_FRAGMENT_SHADER, FRAGMENT_SHADER_NAME)
  ...
  private fun updatePlaneParameters(
    ...
    if (indexBuffer.capacity() < numIndices) {
      var size: Int = indexBuffer.capacity()
    ...
    while (boundary.hasRemaining()) {
      val x: Float = boundary.get()
      val z: Float = boundary.get()
      vertexBuffer.put(x)
      vertexBuffer.put(z)
  ...
  fun drawPlanes(allPlanes: Collection<Plane>, cameraPose: Pose, cameraPerspective: FloatArray) {
    ...
    sortedPlanes.sortWith { a, b -> b?.distance?.compareTo(a?.distance ?: 0f) ?: -1 }
    ...
    for (sortedPlane in sortedPlanes) {
      val plane: Plane = sortedPlane.plane
